...
def library_paper():
    def library_fn(f, x):
        ...
        d4f = partial(vgrad_backward, lambda y: d3f(y)[:, [1]])
        d5f = partial(vgrad_backward, lambda y: d4f(y)[:, [1]])
        
        pred = f(x)
        dt, dx = df(x).T
        u = jnp.concatenate([jnp.ones_like(pred), pred, pred ** 2, pred**3, pred**4], axis=1)
        du = jnp.concatenate(
            [jnp.ones_like(pred), dx[:, None], d2f(x)[:, [1]], d3f(x)[:, [1]], d4f(x)[:, [1]], d5f(x)[:, [1]]], axis=1
        )
        theta = (u[:, :, None] @ du[:, None, :]).reshape(
            -1, 30
        )  # maybe rewrite using vmap?
