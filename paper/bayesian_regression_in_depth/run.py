...
script_dir = "/home/gert-jan/Documents/modax/paper/bayesian_regression_in_depth/runs/"
...
for run_idx, subkey in enumerate(random.split(key, n_runs)):
    print(f"Starting bayes warm run {run_idx}")
    variables = model.init(subkey, X)
    state, params = variables.pop("params")
    ...
    train_max_iter(
        ...
        state,
        max_iterations,
        log_dir=script_dir + f"warm_run_{run_idx}/",
    )

# Running normal PINN
update_fn = create_update(loss_fn_bayesian_ridge, (model, X, y, False))
for run_idx, subkey in enumerate(random.split(key, n_runs)):
    print(f"Starting pinn cold run {run_idx}")
    variables = model.init(subkey, X)
    state, params = variables.pop("params")
    state = (state, {"prior_init": None})  # adding prior to state
    optimizer = optimizer_def.create(params)
    train_max_iter(
        ...
        state,
        max_iterations,
        log_dir=script_dir + f"cold_run_{run_idx}/",
    )
