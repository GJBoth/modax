...
from modax.layers.feature_generators.utils import (
    vgrad_backward,
    vgrad_forward,
    nth_polynomial,
)
...
def BIC(log_p, u_t, theta, alpha, alpha_threshold=1e4):
    ...
    # BIC
    BIC = 2 * log_p + n_samples * jnp.log(reg) + jnp.log(n_samples) * jnp.sum(mask)
...
def loss_fn_SBL(params, state, model, x):
    ...
    (log_p, dt, theta, coeffs), updated_model_state = model.apply(
    ...
    BIC_val, masked_reg, masked_coeffs = BIC(p_mse, dt, theta, prior[:-1])
    metrics = {
        "loss": loss,
        "p_mse": p_mse,
        "p_reg": p_reg,
    ...
    return (
        loss,
        ((updated_model_state, updated_loss_state), metrics, (log_p, dt, theta, mn),),
    )
...
def NF_library(poly_order=2):
    def library_fn(model, inputs):
        ...
        derivs = jnp.concatenate(
            [df_x(X).reshape(-1, 1), d2f_x(X).reshape(-1, 1), d3f_x(X).reshape(-1, 1)],
            axis=1,
        )
